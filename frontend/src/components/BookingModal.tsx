...
export const BookingModal: React.FC<BookingModalProps> = ({ tileId, onClose, onSuccess }) => {
  ...
  // NEW: Track whether the booking API call was successful
  ...
  const handleBooking = async (e: React.SyntheticEvent) => {
    ...
    try {
      ...
      // 1. Show the success view instead of closing immediately
      setIsSuccess(true);

      // 2. Wait 3 seconds, then refresh the map and close the modal
      setTimeout(() => {
        onSuccess();
        onClose();
      }, 3000);
        ...
        {isSuccess ? (
          ...
          <div className="py-8 text-center flex flex-col items-center justify-center animate-in zoom-in duration-300">
            ...
            <p className="text-slate-500 dark:text-slate-400">
              Your cabana ({tileId}) is ready. Returning to map...
            </p>
          </div>
        ) : (
          /* --- FORM STATE --- */
